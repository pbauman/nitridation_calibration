...
namespace NitridationCalibration
{
  ...
  template<class Vec,class Mat>
  ArrheniusGammaNConstantGammaCNLikelihood<Vec,Mat>::~ArrheniusGammaNConstantGammaCNLikelihood()
  {
    return;
  }

  template<class Vec,class Mat>
  void ArrheniusGammaNConstantGammaCNLikelihood<Vec,Mat>::update_parameters( const std::vector<double>& params ) const
  {
    if( params.size() != 3 )
      {
    ...
    gamma_N_params[1] = params[2]*_Ta_N_nom;
  ...
  template class ArrheniusGammaNConstantGammaCNLikelihood<uqGslVectorClass,uqGslMatrixClass>;
