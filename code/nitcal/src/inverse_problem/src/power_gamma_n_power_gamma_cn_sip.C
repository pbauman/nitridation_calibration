...
namespace NitridationCalibration
{
  ...
  template<class Vec,class Mat>
  PowerGammaNPowerGammaCNSIP<Vec,Mat>::~PowerGammaNPowerGammaCNSIP()
  {
    return;
  }

  template<class Vec,class Mat>
  void PowerGammaNPowerGammaCNSIP<Vec,Mat>::create_param_space()
  {
  ...
  template<class Vec,class Mat>
  void PowerGammaNPowerGammaCNSIP<Vec,Mat>::create_param_domain( const GetPot& input )
  {
    // These are assuming normalized values
    const double gamma0_CN_min = input("InverseProblem/gamma0_CN_min", 0.0);
    const double Tref_CN_min = input("InverseProblem/Tref_CN_min", 0.0);
    const double alpha_CN_min = input("InverseProblem/alpha_CN_min", 0.0);
    ...
    param_mins[1] = Tref_CN_min;
    param_mins[2] = alpha_CN_min;
    param_mins[3] = gamma0_N_min;
    param_mins[4] = Tref_N_min;
    param_mins[5] = alpha_N_min;

    Vec param_maxs( this->_param_space->zeroVector() );
    param_maxs[0] = gamma0_CN_max;
    param_maxs[1] = Tref_CN_max;
    param_maxs[2] = alpha_CN_max;
    param_maxs[3] = gamma0_N_max;
  ...
  template<class Vec,class Mat>
  void PowerGammaNPowerGammaCNSIP<Vec,Mat>::create_likelihood( int argc,
                                                               char** argv,
                                                               MPI_Comm mpi_comm,
  ...
  {
    this->_likelihood = 
      new PowerGammaNPowerGammaCNLikelihood<Vec,Mat>( argc, argv, mpi_comm,
                                                      sip_input,
                                                      forward_run_input,
