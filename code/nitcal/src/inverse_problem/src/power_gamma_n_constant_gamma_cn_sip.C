...
namespace NitridationCalibration
{
  ...
  PowerGammaNConstantGammaCNSIP<Vec,Mat>::PowerGammaNConstantGammaCNSIP( uqBaseEnvironmentClass* env,
  ...
  template<class Vec,class Mat>
  PowerGammaNConstantGammaCNSIP<Vec,Mat>::~PowerGammaNConstantGammaCNSIP()
  {
    return;
  }

  template<class Vec,class Mat>
  void PowerGammaNConstantGammaCNSIP<Vec,Mat>::create_param_space()
  {
  ...
  template<class Vec,class Mat>
  void PowerGammaNConstantGammaCNSIP<Vec,Mat>::create_param_domain( const GetPot& input )
  {
    ...
    param_mins[2] = Tref_N_min;
    param_mins[3] = alpha_N_min;

    Vec param_maxs( this->_param_space->zeroVector() );
    param_maxs[0] = gamma_CN_max;
    param_maxs[1] = gamma0_N_max;
    param_maxs[2] = Tref_N_max;
    param_maxs[3] = alpha_N_max;
  ...
  template<class Vec,class Mat>
  void PowerGammaNConstantGammaCNSIP<Vec,Mat>::create_likelihood( int argc,
                                                                  char** argv,
                                                                  MPI_Comm mpi_comm,
  ...
  {
    this->_likelihood = 
      new PowerGammaNConstantGammaCNLikelihood<Vec,Mat>( argc, argv, mpi_comm,
                                                         sip_input,
                                                         forward_run_input,
  ...
  template class PowerGammaNConstantGammaCNSIP<uqGslVectorClass,uqGslMatrixClass>;
